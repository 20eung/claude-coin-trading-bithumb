...
```
"Bithumb API로 BTC/KRW 시장 데이터를 수집하는 Python 스크립트를 만들어줘.
...
```
"Playwright로 Bithumb BTC/KRW 차트를 스크린샷으로 캡처하는 Python 스크립트를 만들어줘.
...
```
"Bithumb API로 내 계좌 잔고를 조회하는 스크립트를 만들어줘.
...
```
"Bithumb API로 시장가 매수/매도를 실행하는 Python 스크립트를 만들어줘.
...
```
"매매 기록을 저장할 SQLite 데이터베이스를 설계해줘.
...
│   ├── db.py                    # DB 모듈
│   └── trading.db                # SQLite DB 파일 (자동 생성)
├── scripts/
│   ├── collect_market_data.py    # Bithumb 시장 데이터 + 기술지표
...
```bash
cd ~/path/to/claude-coin-trading-bithumb && claude
```
...
로컬 SQLite 파일(`database/trading.db`)로 매매 기록을 저장합니다.
...
초기화: `python3 -c "from database.db import init_db_if_not_exists; init_db_if_not_exists()"`
...
| `bithumb-api` | `.claude/skills/bithumb-api/` | Bithumb REST API 래퍼 |
...
|---------------|----------|-------------|-------------|
| 시장 데이터 수집 | `bithumb-api` | `collect_market_data.py` | https://apidocs.bithumb.com |
...
| 포트폴리오 조회 | `bithumb-api` | `get_portfolio.py` | https://apidocs.bithumb.com |
| 매매 실행 | `bithumb-api` | `execute_trade.py` | https://apidocs.bithumb.com |
...
| 소스 | API | 용도 | 인증 |
|------|-----|------|------|
| Bithumb | `api.bithumb.com/v1` | 시세, 호가, 캔들, 매매 실행 | JWT (Access + Secret Key) |
| Alternative.me | `api.alternative.me/fng/` | 공포/탐욕 지수 | 없음 (무료) |
| Tavily | `api.tavily.com/search` | 뉴스 검색 | API Key |
| Playwright | headless Chromium | 차트 스크린샷 캡처 | 없음 |
| SQLite | 로컬 파일 | 데이터 저장/조회 | 없음 |
